...
> 여러 RAG 기법을 **같은 코퍼스·같은 임베딩·같은 생성·같은 judge**로 두고 **검색 메커니즘만 변수**로 비교한 결과의 압축. 과정·타임라인은 [WORKLOG.md](WORKLOG.md), 상세 근거는 [../CLAUDE.md](../CLAUDE.md) §10.
...
## 2. RAG 핵심 결론 (압축)
1. **단일 승자 없다 — 용도별.** 핀포인트/다중홉엔 하이브리드·성숙한 그래프, 종합(global)엔 그래프 계열. → **2 프로필**([PROFILES.md](PROFILES.md)).
