...
- ~~**McNemar 쌍대검정 — 세 방법 전부 유의차 없음**(standard-hybrid p=0.55, standard-bm25 p=0.13, hybrid-bm25 p=0.33, n=100).~~
  > **★ 2026-07-21 교정 (§10.12)** — 이 "전부 동률"은 **결론이 아니라 검정력 부족**이었다.
  > n=400 + 짧은답으로 재측정하니 **의미 ≈ 하이브리드 > 키워드**로 갈렸다
  > (hybrid-bm25 p=0.00001, standard-bm25 p=0.001, standard-hybrid p=0.63).
...
### 10.12 짧은답 프롬프트 + n=400 — 최초의 유의한 우열 (2026-07-21)
> 재설계 Plan A 실행. 상세·전체 표는 [docs/VALIDATION.md](docs/VALIDATION.md) 2·3차 실측.

**(1) 짧은답 프롬프트 — EM/F1 을 논문과 같은 자로.** 공개벤치 정답은 2~3단어 스팬인데 우리는
문장형(평균 20.4단어)으로 답해 EM/F1 이 구조적으로 깎였다. `answer_style: short` 스위치 하나로
전 기법 적용. **원칙: 사전지식 금지 + 컨텍스트 근거만 + 근거 없으면 `NO_ANSWER` 기권.**
("근거 없어도 추정하라"는 초안은 폐기 — 환각 지시이고, 검색 실패를 모델 암기력으로 덮으면
**검색 성능이 아니라 암기력**을 재게 되어 기법 비교 목적이 무너진다.)

| n=100 | judge | EM | F1 | 단어수 |
| --- | --- | --- | --- | --- |
| 전(문장형) | 0.61~0.69 | 0.03~0.12 | 0.19~0.21 | 11.6~20.4 |
| **후(짧은답)** | 0.48~0.59 | **0.37~0.45** | **0.46~0.55** | **1.6~1.8** |

- **F1 이 논문 범위 진입** — standard 0.552 ≈ VLDB Table 5 VanillaRAG 50.8,
  HippoRAG2 0.521 ≈ 논문 50.3. **절대값뿐 아니라 "HippoRAG ≈ Vanilla" 순서까지 재현.**
  → 기존 "우리 0.19 vs 논문 75.5"는 **성능 차가 아니라 답변 형식 차**였음이 확정.
- **judge 하락은 성능 저하가 아니다** — 기권 때문. 답한 문항만 보면 judge 0.855·F1 0.799.
  이전 문장형은 사전지식으로 얼버무려 judge 를 벌었다.
- **부수 발견 — 잡음이 줄자 판별력이 올랐다.** 같은 n=100 에서 McNemar 유의쌍 **0/10 → 3/10**.

**(2) n=400 확대 — 평면 3종 (HotpotQA)**

| 기법 | judge [95% CI] | EM | F1 | recall@4 | 기권 |
| --- | --- | --- | --- | --- | --- |
| **hybrid** | **0.623** [0.575, 0.670] | 0.463 | 0.557 | 0.807 | 28% |
| **standard** | **0.613** [0.565, 0.657] | 0.443 | 0.543 | 0.805 | 30% |
| bm25 | 0.532 [0.485, 0.583] | 0.383 | 0.471 | 0.713 | 37% |

- **★ 프로젝트 최초의 통계적으로 유의한 기법 우열** — hybrid>bm25 **p=0.00001**,
  standard>bm25 **p=0.001**, hybrid vs standard **p=0.63(차이 없음)**.
- **§10.7 교정** — "세 방법 전부 동률"은 결론이 아니라 **검정력 부족**이었다.
  수정: **의미 ≈ 하이브리드 > 키워드**.
- **함의** — 이 벤치에서 하이브리드 이득은 BM25 를 더해서가 아니라 **벡터검색 자체**에서 온다.
  회사 코퍼스의 "하이브리드 > 단일"은 여기서 **재현되지 않음** → §10.7 "도메인 특이적" 판단 **강화**.
- 재현성: [scripts/build_hotpot_bench.py](scripts/build_hotpot_bench.py)(`--n`, 기존 n=100 을
  바이트 일치로 재현) · [scripts/stats_report.py](scripts/stats_report.py)(부트스트랩 CI + McNemar).
- 그래프 2종 n=400 은 인덱싱 진행 중(추출 3,957 / OpenIE 7,914 호출).
